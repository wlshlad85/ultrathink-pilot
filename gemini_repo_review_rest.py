#!/usr/bin/env python3
"""
Gemini Repository Review Script (REST API Version)
Performs a comprehensive code review using Gemini REST API to avoid SSL/gRPC issues.
"""
...
class GeminiRepoReviewerREST:
    """Comprehensive repository reviewer using Gemini REST API."""
    ...
    def review_with_gemini_rest(self, content: str) -> str:
        """Send content to Gemini for review using REST API."""

        prompt = """You are an expert software architect and code reviewer. Perform a comprehensive review of this cryptocurrency trading system repository.
...
"""
        ...
        try:
            ...
            print(f"🔄 Calling Gemini REST API (call #{self.stats['api_calls']})...")
    ...
    def run_full_review(self, repo_path: str = ".", output_file: str = None) -> Dict[str, Any]:
        ...
        print("🚀 Starting Gemini Repository Review (REST API)...")
        ...
        gemini_review = self.review_with_gemini_rest(review_content)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gemini Repository Review (REST API)")
    parser.add_argument(
        '--repo-path',
        default='.',
    ...
    try:
        reviewer = GeminiRepoReviewerREST(api_key=args.api_key, model=args.model)
