...
class SimpleRewardCalculatorExp:
    """
    ...
    def __init__(
        ...
        volatility_sensitivity: float = 50.0,  # EXPONENTIAL DECAY
    ...
    def calculate_reward(
        ...
        # === STEP 2: RISK ADJUSTMENT (EXPONENTIAL DECAY) ===
        if len(self.returns_history) >= self.min_samples_for_volatility:
            returns_array = np.array(list(self.returns_history))
            volatility = np.std(returns_array)

            # Stability factor: EXPONENTIAL decay for volatile performance
            # High volatility → exponentially low factor → heavily reduced reward
            # Low volatility → factor ≈ 1.0 → full reward
            # Formula: exp(-vol * sensitivity) instead of 1/(1+vol*sensitivity)
            stability_factor = np.exp(-volatility * self.volatility_sensitivity)
